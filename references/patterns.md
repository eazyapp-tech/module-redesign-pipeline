...
- **One flow, two ways to approve** (payment page, 24 Sep 2026). Problem: e-NACH and UPI Autopay share their choices, dates and promises, but not the words about approval, PIN, app or timing. The house's answer is one words table, `RAILS` plus `railOf(s, query)` in `components/PayPage/autopay.js`, which every shared screen reads. The route carries `rail` in the address while she sets up; the mandate carries it once it is on. A test checks that both routes have the same keys, so a new UPI line cannot ship without its bank version. Don't use it for words that are the same on both routes.
- **State that is set together is held together** (payment page, 24 Sep 2026). Problem: a refusal was three useStates (sentence, rule flag, lower figure); two of five writers updated one and left two, so a stale "Pay ₹X instead" door survived onto a different refusal (#1069). Answer: one `refusal` object `{ text, rule, lower }`, written whole, with the old name kept as a derived read (`const problem = refusal?.text || null`), in `pages/_sites/payment-pages/p2/[shortId]/pay.js` and `components/PayPage/Recharge.jsx`. Proven by stubbing the server calls with Playwright `page.route` and walking the sequence on the old code first.
- **A checkbox row with a multi-line label aligns to the first line** (payment page, 24 Sep 2026). Centring the box and a trailing link on a three-line label floats both beside a middle word. Answer: box, first line and link on one shared line-height at the top; the link's box stretches to the row height for its tap area (`.pay-tick*` in public/paypage/pay-page.css).
- **An inherited CSS property is set once on the page** (payment page, 24 Sep 2026). Five classes each carried `text-wrap: pretty` and fifteen lines still ended on one word; `body { text-wrap: pretty }` in pay-page.css reaches every line, sheets included, and a heading's own `balance` still wins.
- **A chapter laid over a hidden full stage keeps a sheet's height** (payment page, 24 Sep 2026). A short first chapter before a tall announcement made the sheet grow on Next. The house's answer: keep the announcement mounted with `visibility: hidden` (in place, unfocusable) and lay the chapter over it (`position: absolute; inset: 0`), so the sheet is the announcement's height at every width with no measured constant (`.pay-launch-frame`, `.pay-launch-look` in pay-page.css; AutopayCard.jsx LaunchSheet). Switch the drawer's `labelledBy` and `focusRef` to the chapter while it shows, and move focus to the announcement's heading on Next.
- **A date picker draws the dates the money will use.** Problem: day pills tagged "1 Oct" while the first debit was 5 Nov, because the label was worked out separately from the schedule. Answer: build every drawn date from the scheduling function itself, one call per choice (`windowDates` calls `autopayPlan` per day, components/PayPage/autopay.js, pay-autopay worktree), and dedupe dates a short month collapses. (24 Sep, step 2 calendar)
- **A state word comes from the next event, not from a record existing** (payment page, 24 Sep 2026). "Paused" was `a.pause ? 'paused'`, so a skip two months out would have greyed her autopay today and swapped the key to Resume. Answer: paused means her next debit falls inside the pause (`pausedNow`, components/PayPage/autopay.js, pay-autopay worktree); a finished pause is no pause.
- **A swipe-revealed control has a visible twin** (payment page, 24 Sep 2026). The month card turns on a swipe, and a "Card controls" pill under it opens the same controls, moves focus to the first, and Escape closes it. A swipe alone locks out keyboard, screen-reader and "I did not know you could swipe" users.
- **One decision about what is on screen is a sheet; a sequence is a screen** (payment page, 24 Sep 2026). Change date, Pause, Skip, Resume, Cancel and the check screen's Change links each opened a full page, so she lost the card she was acting on. Answer: keep the step in the URL (back button and links work) but draw it as the page's Drawer over the screen it acts on; confirming closes the sheet and the object changes in place, which is the feedback. A multi-step setup stays full screen.
- **A test bench anyone can open** (payment page, 25 Sep 2026). Problem: testing a money flow needs real links (they expire in 7 days and carry real tenants), a real gateway (real money) and a person to set it up. Answer: capture one real API answer per case, make it anonymous (scripts/paypage/demo-capture.mjs, which ends by printing anything still shaped like a phone, an email or an id), and answer the page's own few call sites from those files when the code is a demo code (components/PayPage/demo.server.js, pages/api/pay-demo.js). Dates move with the calendar; what a tester pays is a cookie applied on the next load; the gateway is a pretend app that returns to the real return URL, so everything after payment is the real code. One env var: PAY_DEMO=only refuses every real link, so the address can be shared. The same tenants feed the automated launch check.
- **The thing she chose travels with the money, and the receipt names only that** (payment page, 25 Sep 2026). Problem: a Pay key on one bill sent no bill, the backend matched by amount, and the receipt listed her open bills. Answer: one decision for "which bill" used by both the payment and the screen that shows it (pay.js `bill`), one function that speaks the backend's words (components/PayPage/money.api.js `billsFor`), and a receipt title that names what was sent or says it does not know ("Towards your dues"). A named bill that is gone names nothing, never a neighbour.
- **The figure on a key is the figure the next screen opens at** (payment page, 25 Sep 2026). Problem: "Total dues ₹35,393 · Pay all" opened at ₹23,193 because the pay screen chose its own opening figure (the link's amount) and never read which key sent her; four keys had the same fault. Answer: every key puts its intent in the address (`?bill=`, `?all=1`, `?how=cash`), the destination computes its figure from that intent in one function (`openingAmount`, components/PayPage/state.js), and the stored default (the link's amount) is used only when no key was pressed. Sweep by listing every caller of the destination route, not the key named in the report.
- **A demo stand-in models the server's rule, not the happy path** (payment page, 25 Sep 2026). Problem: the demo bench put money on "the bill she named, then oldest first", so a payment the real backend would land on the wrong bill (#1087: autopay's rent taken by a same-amount match) looked right to every tester. Answer: the stand-in takes the exact payload the real call sends and applies the backend's own rule in one function (`reach`, eazypg-marketplace components/PayPage/demo.server.js: the list confines, the named bill goes first, same amount alone, then oldest), with a test that reproduces the real bug on it. When a stand-in answers a money call, read the server's code for the rule before writing it.
- **When what she came for is gone, say so before offering anything else** (payment page, 26 Sep 2026). Problem: a link for one bill, an old pay-screen address and an old bill page all quietly fell back to "everything she owes" once that bill was paid. Answer: one lookup of what became of it (`billGone`, eazypg-marketplace components/PayPage/state.js: still owed, paid with its record, or unknown), one place that says it (the bill, above its tabs, with the receipt one tap away), and every other entrance sends her there (`?was=`). Paid from a deposit is "settled", not "paid"; where the lookup cannot be sure (merged rows), it says nothing rather than something false.
