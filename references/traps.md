...
- **`codex exec` waits on stdin, and reviews at low effort by default** (25 Sep 2026). Run in the background, it printed "Reading additional input from stdin..." and sat for ten minutes producing nothing, which looks like a slow review. Close stdin and set the effort for money paths: `codex exec -c model_reasoning_effort=high --sandbox read-only -C <repo> "<prompt>" < /dev/null`. Check the output file grows before waiting on it.
- **A slow-network check on the dev server finds silences production does not have** (payment page, 25 Sep 2026). On the first client navigation `next dev` fetches `_devPagesManifest.json`; at 1.5s latency a cash key read silent for 2s on dev and answered in 100ms on the production build. Run tap-response checks against a production build (the demo tunnel).
- **run_probe.py `--wait` is seconds, not milliseconds** (25 Sep 2026). `--wait 3000` sat on the first page for 50 minutes with no output. Use `--wait 3`: `python3 scripts/run_probe.py --url <url> --widths 375,1440 --wait 3`.
- **An invoice's amount today is not its amount last week** (payment page, 25 Sep 2026). Automatic Late Fine rows grow daily or monthly after they are raised, and rows carry no updated_at, so any rule that sums "bills that existed then" at today's amounts overcounts. Measured: counting fines nearly doubled the links that looked like part asks (33,229 against 17,596 of 221,760). Leave fines out of any then-versus-now comparison (eazypg-marketplace components/PayPage/autopay.js askedFor).
- **A fixed date in a demo address goes stale overnight** (payment page, 26 Sep 2026). The demo's dates move with the calendar, so `?autopay=on&day=30` covered a rent due on the 30th one day and not the next, when the rent fell due on the 1st; the test-plan journey silently stopped showing what it promised. Leave the day out and let the preview pick it (`withPreview` in components/PayPage/autopay.js), or derive it from the tenant.
