...
| 1 | **Ground** | Component registry first, then `Explore` subagents (2–5 in parallel) for the gaps | **Registry-first:** read `COMPONENT-REGISTRY.md` and verify the entries this module will touch (paths still real, components unchanged — a stale registry is worse than none). Explore agents then map only what the registry doesn't cover: this module's own end-to-end flow and real data shapes. Anything reusable they discover gets written INTO the registry, not discarded with the session. No registry yet → this phase's output seeds it |
...
| 5 | **Build** | `interface-design`, executed via `superpowers:subagent-driven-development` | Strict per-task loop: Implement Task N → Review Task N (spec + quality) → Fix → Re-review, fanned to `general-purpose` subagents. **Registry check before any new component** — found → adapt per its when-NOT-to-use and gotcha notes (never literal-copy); on the do-NOT-copy list → use the named replacement; genuinely absent → one Explore confirm, then build. Give build subagents the registry's relevant entries in their prompt so they reuse instead of parallel-invent |
...
**Round close updates the registry, every time:** new reusable components built this round get registered (an unregistered reusable is the seed of the next duplicate); code-truth gotchas learned this round get a dated entry; near-duplicates spotted go on the extraction-candidates list (3+ call sites → extract to canonical). Tooling gotchas (login quirks, viewport routing) go in the ledger's verify recipe instead — the registry is about the code, not our tools.
