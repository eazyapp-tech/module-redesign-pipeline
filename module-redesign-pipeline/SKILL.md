...
| 6 | **Motion** | `emil-design-eng` | Micro-interaction pass. `apple-design`, `animation-vocabulary`, `review-animations`, `improve-animations` are deeper motion tools — reach for them when needs exceed `emil-design-eng`'s coverage (they weren't part of the origin sessions) |
...
| 8 | **Final gate (parallel)** | `impeccable` ×2 + `web-design-guidelines` + `code-review --high` + `agent-skills:code-reviewer` | Runs on **acceptance-candidate rounds only** — interim rounds end at Phase 7. Three dimensions graded separately, never folded: **design-craft** (two parallel `general-purpose` subagents, each given the live URL/screenshots and impeccable's critique flow, no shared context), **guidelines-compliance** (`web-design-guidelines` against the FINAL code — mandatory for web), **code-quality** (`code-review --high`, 8 angles incl. removed-behavior checked against the feature-parity matrix; plus a fresh context-free `code-reviewer` pre-merge pass) |
